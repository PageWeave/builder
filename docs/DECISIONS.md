...
**Consequences:** engine chunk ships inside `out/` (M5 packaging: no node_modules needed for the engine). Typecheck uses a local stub for `pi-mcp-adapter` (tsconfig `paths` redirect — its raw-TS types entry would drag the whole source graph, incl. an uninstalled peer, into our strict check; builds are unaffected since Vite doesn't read tsconfig paths).
