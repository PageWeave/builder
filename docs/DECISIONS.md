...
## D17 — Packaging & distribution: electron-builder + GitHub-Releases update feed, hardened fuses, deps split.

**Context (M5):** shipping mac + Windows installers with auto-update needs a builder, a feed, and a signing story. The app's runtime surface is fully bundled by electron-vite (D12/D14) — node_modules shipping should be minimal; renderer deps never belonged in `dependencies` anyway (electron-vite skill red flag). Platform-side task "decide auto-update feed host" (SPEC-PLATFORM) had to land.

**Decision:** electron-builder 26.15.3 + electron-updater 6.8.9 (exact pins). Feed = **GitHub Releases drafts** (`PageWeave/builder`), not a pageweave.dev static route — zero platform work, electron-updater's built-in signature verification, staged rollout via draft → inspect → publish. Revisit only if branded download URLs become a requirement (release.yml is the single pivot point). **Deps split:** only `@silvia-odwyer/photon-node` stays in `dependencies` (the one externalized runtime import, D14); pi packages, MCP SDK, openid-client, react, react-dom, stream-md move to devDependencies — the asar ships the bundle + photon only (18 MB). **Installers:** macOS universal dmg (one artifact for both archs), Windows NSIS x64 one-click per-user (non-technical users: no admin prompt, no install-dir decision). **Fuses (hardened):** RunAsNode/NodeOptions/NodeCliInspect/FileProtocolExtraPrivileges OFF; CookieEncryption, EmbeddedAsarIntegrity + OnlyLoadAppFromAsar ON (the pair is required — one alone is bypassable). Known trade-off: NodeCliInspect OFF blocks future *packaged*-app E2E; the suite runs the unpackaged build (testing skill). **Update UX:** check 5 s after boot + every 4 h, silent download, install on quit, restart prompt dialog; Help → "Check for Updates…" for manual checks. No updater in dev/E2E/smoke (`app.isPackaged` + `PW_SMOKE` guard).

**Alternatives rejected:** pageweave.dev static feed (platform work + we'd have to re-implement/verify feed signatures ourselves); electron-updater `autoInstallOnAppQuit` alone with no prompt (users miss updates for weeks); arm64+x64 split dmgs (two downloads to reason about); electron-forge (we'd rewrite what electron-builder gives: dmg/NSIS/notarize/publish).

**Consequences:** signing is env-gated in release.yml (CSC_*/APPLE_* secrets — docs/RELEASE.md inventory); unsigned drafts still build when secrets are absent so the pipeline is testable before certs arrive; CI gained a `package` job that boots the real asar bundle to catch packaging regressions; `author` had to be added to package.json (NSIS metadata); the app-id CONFIRM (`dev.pageweave.builder`) is adopted as-is.
