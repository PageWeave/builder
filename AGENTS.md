...
M4 done (2026-09-18): conversation surfaces (resume/switch/fresh open-session, list-sessions, history backfill) + engine-side official-MCP-SDK site client (list_websites/get_website, D15); three-pane shell with site picker, first-run gates, product chat (stream-md per D16), confirmation cards, menu-toggled debug console (Cmd/Ctrl+Shift+D), preview pane (main-process WebContentsView, persist partition, fenced nav, debounced auto-refresh on edit events); Playwright _electron E2E (5 specs, PW_E2E=1 IPC stubs) with its own CI job. 73 unit tests + 5 E2E specs. M4 acceptance run (real key + real edits) still pending. M5 plumbing done (2026-09-18, D17): electron-builder 26.15.3 (universal dmg + NSIS one-click per-user, appId `dev.pageweave.builder`, hardened fuses incl. ASAR integrity), electron-updater 6.8.9 on GitHub-Releases draft feed (silent download, install on quit, Help-menu check), release.yml tag pipeline (unsigned drafts until CSC/APPLE secrets land — docs/RELEASE.md § secrets), CI `package` job boots the real asar bundle. Deps split: only `@silvia-odwyer/photon-node` in `dependencies` (externalized wasm loader); everything else devDeps — keep it that way. BLOCKED on user: signing certs, acceptance run, app icon (build/icon.png). M3 remains the last full milestone — engine live on top of M1+M2 — pi packages exact-pinned (`@earendil-works/pi-coding-agent` 0.85.1, `pi-ai` 0.85.1, `pi-tui` 0.85.1, `pi-mcp-adapter` 2.34.0), ALL imports confined to `src/engine/` (R2). Embedding per D14: `ModelRuntime` + `InMemoryCredentialStore` (BYOK keys in memory only, safeStorage in main), MCP via `createMcpAdapter` isolated config (bearer via `${PW_ACCESS_TOKEN}` env interpolation, lazy lifecycle, `directTools: true` + `freezeDirectTools`), token rotation via main→engine `token-updated` push, per-website sessions under `<userData>/work/<siteId>` with `PI_CODING_AGENT_DIR=<userData>/agent` (never `~/.pi`), bundled skills (`npm run sync:docs` → `src/engine/resources/skills/`), engine chunk bundled single-file (`externalizeDeps: false` + `inlineDynamicImports`, 15 MB — adapter ships raw TS, externalizing is impossible). Debug chat console in renderer; 49 tests; CI green. Live acceptance run (model key + account) still pending — PLAN.md § Next actions. `pi-mcp-adapter` is typecheck-stubbed via tsconfig `paths` (raw-TS types entry). Agent skills in `.opencode/skills/` — read the relevant one before touching its area.
